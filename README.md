...
## Caché (Supabase) — clave unificada `cache_key`

- Producto con barcode → `cache_key = <barcode>`.
- Producto resuelto **solo por IA** (búsqueda por nombre sin match en OFF) →
  `cache_key = 'name:<nombre normalizado>'` (minúsculas, sin acentos, espacios
  colapsados) y `barcode = null`. La segunda búsqueda idéntica se sirve del
  cache **sin gastar IA**.
- Upsert `onConflict:'cache_key'`; se guardan los CRUDOS (`ingredients_text`,
  `nutriments`, `nova_group`, `additives_tags`) + denormalizados para listados.
  El score se **recomputa al leer** (puede cambiar entre versiones del motor —
  columna `engine_version`).
- Unicidad: `UNIQUE(cache_key)` y `UNIQUE(barcode)`. `product_name` **no** es
...
**Unit tests (68):** co-locados como `src/**/*.test.ts`. Cubren, entre otros:
...
- Cache: round-trip de `buildCachePayload`/`getCachedProduct`, filas viejas sin
  crudos tratadas como miss, clave `name:` con barcode null.
...
- `POST /products/lookup` `{ query }` — lookup principal. **TODO:** reactivar
  `requireAuth` (hoy abierto para testing).
